...
const STATUS_BADGE: Record<string, string> = {
  ...
  available: 'bg-indigo-500/15 text-indigo-300',
...
function UtilBar({ util }: { util: SubnetUtil }) {
  ...
  return (
    <div className="space-y-1">
      ...
      <div className="text-[11px] text-indigo-400">{util.available} available</div>
    </div>
  )
}
              ...
              <div className="flex flex-wrap gap-3">
                {configuredNets.map((n) => (
                  <form key={n.cidr} onSubmit={(e) => handleQuickAdd(e, n.cidr)}>
                    <button type="submit" className="flex items-center gap-2 rounded-xl border border-emerald-500/30 bg-slate-950/60 px-3 py-2 text-sm transition hover:border-indigo-400">
          ...
          {containerNets.length > 0 && (
            <div>
              ...
              <div className="flex flex-wrap gap-3">
                {containerNets.map((n) => (
                  <form key={n.cidr} onSubmit={(e) => handleQuickAdd(e, n.cidr)}>
                    <button type="submit" className="flex items-center gap-2 rounded-xl border border-slate-700/60 bg-slate-950/40 px-3 py-2 text-sm transition hover:border-indigo-400">
              ...
              <p className="mt-3 text-[11px] text-slate-500">To always show specific subnets here, add <code className="rounded bg-slate-800 px-1 py-0.5 text-indigo-300">SCAN_NETWORKS=192.168.1.0/24</code> to your <code className="rounded bg-slate-800 px-1 py-0.5 text-indigo-300">.env</code> and restart.</p>
            </div>
          )}
        </div>
      )}
